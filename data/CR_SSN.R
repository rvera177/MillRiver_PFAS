...
#now that i Have the starting comid, I collect all flowlines upstream of my start point
...
CR_ssn <- ssn_assemble(
  ...
  ssn_path = paste0(temp_dir, "/CR.ssn"),
  import = TRUE,
  check = TRUE,
  afv_col = "afvArea",
  overwrite = TRUE
...
for (compound in pfas_cols) {
  response <- CR_ssn$obs[[compound]]  # extract the response values
...
for (compound in pfas_cols) {
  ...
  df <- preds[[compound]] %>%
    ...
    left_join(
      CR_ssn$preds$preds %>% st_drop_geometry() %>% select(pid, comid),
      by = "pid"
    ) %>%
    select(comid, .fitted) %>%
    rename(pred = .fitted)
  
  CR_pred$edges <- CR_pred$edges %>%
